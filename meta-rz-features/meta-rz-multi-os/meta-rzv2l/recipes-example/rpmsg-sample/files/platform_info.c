...
//    rproc_priv->vr_info = &vrinfo[rsc_index];
    ...
    rsc_table = remoteproc_mmap(rproc_inst, &pa,
                ...
                0, NULL);
    if (!rsc_table) {
        LPRINTF("\n RPMSG_INFO:Failed to map the resource table.");
        goto err2;
    }
#else /* uC3 */
    ...
    if (ret) {
        LPRINTF("\n RPMSG_INFO:Failed to intialize remoteproc");
        goto err2;
    }
    g_rsc_table = rsc_table;
    g_rsc_io = rproc_inst->rsc_io;
skip:
    LPRINTF("\n RPMSG_INFO:Initialize remoteproc successfully.");
               ...
               rpmsg_ns_bind_cb ns_bind_cb)
{
    ...
    LPRINTF("\n RPMSG_INFO:creating remoteproc virtio");
    vdev = remoteproc_create_virtio(rproc, (int)vdev_index, role, rst_cb);
    if (!vdev) {
        LPRINTF("\n RPMSG_INFO:failed remoteproc_create_virtio");
        goto err;
    }
    ...
    if (!shbuf_io) {
        LPRINTF("\n RPMSG_INFO:failed remoteproc_get_io_with_pa");
        goto err;
    }

    /* Only RPMsg virtio master needs to initialize the shared buffers pool */
#ifdef __linux__
    LPRINTF("\n RPMSG_INFO:initializing rpmsg shared buffer pool");
...
int platform_poll(struct remoteproc *rproc)
{
    ...
    while(!force_stop) {
        // LPRINTF("\n RPMSG_INFO:________1______Inside platform_poll in platform_info.c");
        ble_queue_reception();
        flags = metal_irq_save_disable();

        if (!(atomic_flag_test_and_set(&ipi.sync))) {
            metal_irq_restore_enable(flags);
            LPRINTF("\n RPMSG_INFO:______2________Inside if condition platform_poll in platform_info.c");
            remoteproc_get_notification(rproc, RSC_NOTIFY_ID_ANY);
            break;
        }
        
        // LPRINTF("\n RPMSG_INFO:______3________Outside if condition platform_poll in platform_info.c");
        metal_irq_restore_enable(flags);
        // LPRINTF("\n RPMSG_INFO:______4________Outside if condition platform_poll in platform_info.c");   
        pthread_mutex_lock(&mutex);
        // LPRINTF("\n RPMSG_INFO:_____5________Outside if condition platform_poll in platform_info.c");
        /* Abhinav */
        //pthread_cond_wait(&cond, &mutex);
        // LPRINTF("\n RPMSG_INFO:______6________Outside if condition platform_poll in platform_info.c");
        pthread_mutex_unlock(&mutex);
        // LPRINTF("\n RPMSG_INFO:_______7_______End of platform_poll in platform_info.c");
    }
#else /* uC3 */
    (void) rproc;
#endif
...
int platform_init(unsigned long proc_id, unsigned long rsc_id, struct remoteproc **platform)
{
    ...
    if (!platform) {
        LPRINTF("\n RPMSG_INFO:Failed to initialize platform," "NULL pointer to store platform data.");
        return -EINVAL;
    }

    if ((proc_id >= RPVDEV_MAX_NUM) || (rsc_id >= RPVDEV_MAX_NUM)) {
        LPRINTF("\n RPMSG_INFO:Invalid rproc number specified.");
        return -EINVAL;
    }

    rproc = platform_create_proc((int)proc_id, (int)rsc_id);
    if (!rproc) {
        LPRINTF("\n RPMSG_INFO:Failed to create remoteproc device.");
        return -EINVAL;
    }
    *platform = rproc;
...
static void IpiTaskId(VP_INT exinf)
{
    ...
    while (1) {
        ...
        if (ret) {
            LPRINTF("\n RPMSG_INFO:remoteproc_get_notification() failed with %d", reg);
        }
    }
}
...
static void start_ipi_task(void *platform)
{
    ...
    if (ipi_tsk_id[prproc->notify_id] <= 0) {
        LPRINTF("\n RPMSG_INFO:Failed to register an interrupt service routine.");
    }
}
#endif
