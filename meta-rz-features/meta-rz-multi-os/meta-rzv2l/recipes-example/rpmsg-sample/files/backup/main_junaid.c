...
typedef struct {
    ...
    double Temperature_value; // 8 bytes
    double loadcell_Value;    // 8 bytes
} SensorData;
...
static int rpmsg_service_cb0(struct rpmsg_endpoint *cb_rp_ept, void *data, size_t len, uint32_t src, void *priv) {
    ...
    // Ignore data if session is not active
    if (!session_active) {
        return 0;
    }
    ...
    if (buffer_size + sensor_str_size > TARGET_SIZE) {
        ...
        snprintf(filename, sizeof(filename), "%s/%s_file%d.csv", FOLDER_NAME, session_uuid, file_counter);
...
static int payload_init(struct rpmsg_device *rdev, struct payload_info *pi) {
    ...
    i_payload = (struct _payload *)metal_allocate_memory(2 * sizeof(unsigned long) +
                                                         pi->maxnum);
    if (!i_payload) {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        printf("Received command ID: %02X\n", command_id);
