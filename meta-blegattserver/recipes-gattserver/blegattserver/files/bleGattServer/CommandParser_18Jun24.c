...
void parseCommand(const uint8_t *data, int data_length, void (*readCallback)(const uint8_t *, size_t), void (*notifyCallback)(const uint8_t *, size_t))
{
    ...
    case 0x10:
        ...
        msg.message_text[0] = 0x01;    // First byte is the command ID
        msg.message_text[1] = data[4]; // Start /Stop
        // memcpy(msg.message_text + 1, data + 4, data_length - 4); // Copy the rest of the data// Ensure the message size is within limits

        if (msgsnd(msgid, &msg, sizeof(msg.message_text), 0) == -1)
        {
            printf("\n BLE_HOOK_INFO :Error, Not able to send harvest start/stop command to M33 core.\n");
            perror("msgsnd failed");
            exit(EXIT_FAILURE);
        }
        else
        {
            printf("\n BLE_HOOK_INFO : send harvest start/stop command  sent to M33 Core = %s\n", msg.message_text);
    ...
    case 0x21: // load cell tare
        ...
        msg.message_type = 1;       // Message type can be used to differentiate messages if needed
        msg.message_text[0] = 0x02; // First byte is the command ID
        msg.message_text[1] = 0;

        if (msgsnd(msgid, &msg, sizeof(msg.message_text), 0) == -1)
        {
            printf("\n BLE_HOOK_INFO :Error, Not able to send load cell tare command to M33 core.\n");
            perror("msgsnd failed");
            exit(EXIT_FAILURE);
        }
        else
        {
            printf("\n BLE_HOOK_INFO : send load cell tare command  sent to M33 Core = %s\n", msg.message_text);
            printf("\n");
        }
    ...
    case 0x22: // load cell start calibrate
        ...
        msg.message_type = 1;          // Message type can be used to differentiate messages if needed
        msg.message_text[0] = 0x03;    // First byte is the command ID
